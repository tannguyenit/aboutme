...
class Public_news extends MY_Controller {
	...
	public function index($page=1)
	{
		...
		foreach ($arData["arCat"] as $key => $value) {
			//echo $value["name"];
			//$arData["arNews"]=$this->news_model->getid_cat($value["id_cat"]);
			//cho "</pre>";
		}
	...
	{
		// if ($this->session->userdata('id_news')) {
		// 	$id_news_cu=$this->session->userdata('id_news');
		// 	$id_news=$id_news_cu["id"];
		// }
	...
	public function read()
	{
		...
		// }else{
		// 	$array = array('id' => $id);
		// 	$this->session->set_userdata("id_news",$array );
		// }
		// $array = array('id' => $id);
		//$this->session->set_userdata("id_news",$array );
		$arData['arNews']=$this->news_model->detail($id);
		$read=$arData['arNews']["read"];
		$arRead=array("read"=>$read+1);
